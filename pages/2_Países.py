...
options = st.sidebar.multiselect (
    ...
    ['Brazil','Canada','England','Australia','Qatar','India','United States of America',
     'United Arab Emirates','Philippines','Turkey'] )
...
tab1, tab2 = st.tabs(["Visão Gráfica", "Visão Tabela"])


tab1.header('_Visão Países_')
...
with st.container ():  
    with tab1:    
        ...
        with col1:
            ...
            fig=px.line(df1,x='country_code',y='city')
        ...
        with col2:
            ...
            df1 = (df.loc[:,['country_code','restaurant_id']].groupby('country_code').count()
                                                .sort_values('restaurant_id',ascending=False)
                                                .reset_index()).head(15)
            fig=px.line(df1,x='country_code',y='restaurant_id')
            fig .update_layout(xaxis_title='Países')
        ...
        with col1:
            st.subheader('Países com Maior Quantidade de Avaliações')
            df1 = df.loc[:,['country_code','votes']].groupby('country_code').sum().sort_values('votes',ascending=False).reset_index()
            fig=px.line(df1,x='country_code',y='votes')
            fig .update_layout(xaxis_title='Países')
            fig .update_layout(yaxis_title='Quantidade de Avaliações')
            st.plotly_chart(fig, use_container_width=True)

        with col2:
            
            st.subheader('Países com mais Tipos de Culinárias distintos')
            df1 = df.loc[:,['country_code','cuisines']].groupby('country_code').nunique().sort_values('cuisines',ascending=False).reset_index().head(15)
            fig=px.line(df1,x='country_code',y='cuisines')
            fig .update_layout(xaxis_title='Países')
            fig .update_layout(yaxis_title='Quantidade de Culinárias')
            st.plotly_chart(fig, use_container_width=True)


tab2.header('_Visão Países_')
with st.container():
    with tab2:    
